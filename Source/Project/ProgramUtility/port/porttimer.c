...
ULONG           ulTimeOut;
BOOL            bTimeoutEnable;
ULONG           ulMasterTimeOut;
BOOL            bMasterTimeoutEnable;

static struct timeval xTimeLast;
static struct timeval xMasterTimeLast;
...
vMBPortTimerPoll(  )
{
    ...
    if( bTimeoutEnable )
    {
        ...
        else
        {
            ...
            {
                DEBUG_PUTSTRING1("T35 delta time=",ulDeltaMS);
                bTimeoutEnable = FALSE;
                ( void )pxMBPortCBTimerExpired(  );
            }
        }
    }
    if( bMasterTimeoutEnable )
    {
        if( gettimeofday( &xTimeCur, NULL ) != 0 )
        {
            /* gettimeofday failed - retry next time. */
        ...
        {
            ulDeltaMS = ( xTimeCur.tv_sec - xMasterTimeLast.tv_sec ) * 1000L +
                ( xTimeCur.tv_usec - xMasterTimeLast.tv_usec ) / 1000L;
            if( ulDeltaMS > ulMasterTimeOut )
            {
                bMasterTimeoutEnable = FALSE;
                DEBUG_PUTSTRING1("Master delta time=",ulDeltaMS);
                ( void )pxMBPortCBMasterTimerExpired(  );
            }
        }
    }
}
